...
test('homepage fragments resolve in built markup, including no-JS links', () => {
  const result = check(
    ...
    '<a href="https://metrale.ai/#verified">Benchmarks</a><a href="https://metrale.ai/index.html#run">Start Atlas</a>'
  );
...
test('removing every cross-property fragment link still fails the guard', () => {
  const result = check(
    ...
    '<a href="https://metrale.ai">Atlas</a><a href="/#verified">Local section</a>'
  );
  expect(result.status).toBe(1);
