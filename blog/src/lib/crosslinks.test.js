...
test('a fragment on the engine page cannot satisfy a homepage link', () => {
  const result = check(
    ...
    '<a href="https://metrale.ai/#verified">Benchmarks</a>'
  );
  expect(result.status).toBe(1);
  expect(result.stderr).toContain('https://metrale.ai/#verified');
});

test('homepage fragments resolve in built markup, including no-JS links', () => {
  const result = check(
    { 'index.html': '<section id="verified"></section><noscript><a id="run" href="/engine.html#run">Start the engine</a></noscript>' },
    '<a href="https://metrale.ai/#verified">Benchmarks</a><a href="https://metrale.ai/index.html#run">Start the engine</a>'
  );
...
test('explicit engine links resolve against their own built page', () => {
  const result = check(
    { 'index.html': '<main id="home"></main>', 'engine.html': '<section id="verified"></section>' },
    '<a href="https://metrale.ai/engine.html?source=blog#verified">Benchmarks</a>'
  );
  expect(result.status).toBe(0);
...
test('URL-encoded fragments match the browser destination', () => {
  const result = check(
    ...
    '<a href="https://metrale.ai/#model%2Ddetail">Models</a>'
  );
  expect(result.status).toBe(0);
});
...
test('extensionless /engine resolves against engine.html ids', () => {
  const result = check(
    { 'index.html': '<main id="home"></main>', 'engine.html': '<section id="verified"></section>' },
    '<a href="https://metrale.ai/engine#verified">Benchmarks</a>'
  );
  expect(result.status).toBe(0);
  expect(result.stdout).toContain('all 1 cross-property fragment links resolve');
});
...
test('a missing fragment on /engine still fails after the pretty-URL alias', () => {
  const result = check(
    { 'index.html': '<main id="home"></main>', 'engine.html': '<section id="verified"></section>' },
    '<a href="https://metrale.ai/engine#nope">Benchmarks</a>'
  );
  expect(result.status).toBe(1);
  expect(result.stderr).toContain('https://metrale.ai/engine#nope');
});

test('removing every cross-property fragment link still fails the guard', () => {
  const result = check(
    { 'index.html': '<main id="home"></main>' },
    '<a href="https://metrale.ai">Metrale</a><a href="/#verified">Local section</a>'
  );
  expect(result.status).toBe(1);
  expect(result.stderr).toContain('No https://metrale.ai');
});
