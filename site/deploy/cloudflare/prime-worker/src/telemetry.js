...
export async function record(env, entry) {
  ...
  try {
    await env.PRIME.put(`log:${at}:${entry.id}`, JSON.stringify({ ...entry, at }), days > 0 ? { expirationTtl: Math.round(days * 86400) } : undefined);
  } catch {
...
export async function stats(env, { days = 7, max = 2000 } = {}) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    const ttft = ok.map((r) => r.ttft_ms).filter(Number.isFinite).sort((a, b) => a - b);
    const total = ok.map((r) => r.total_ms).filter(Number.isFinite).sort((a, b) => a - b);
    ...
    out.push({
      ...
      total_ms: { p50: pct(total, 50), p95: pct(total, 95), avg: total.length ? Math.round(total.reduce((a, b) => a + b, 0) / total.length) : 0 },
  ...
  return { generated: new Date().toISOString(), spent_today_usd: round(await spentToday(env), 4), budget_usd: Number(env.PRIME_DAILY_BUDGET_USD ?? 0), answers: all.length, days: out, by_model: byModel, tools };
}
