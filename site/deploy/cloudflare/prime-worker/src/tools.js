...
import { fleetModel, apiModel, energyModel, energyInputsFrom, FLEET_DEFAULTS, API_DEFAULTS, ENERGY_DEFAULTS } from '../../../../src/lib/economics.js';
...
export const TOOLS = [
  {
    ...
    function: {
      name: 'search_site',
      description: 'Search everything Metrale has published: the pages of this website, the engine documentation in the repository, the blog, and the repository history. Returns numbered passages to cite as [n]. Search again with different words if the first results are thin.',
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'What to look for, in plain words. Five to twelve words works best.' },
          kinds: { type: 'array', items: { type: 'string', enum: ['page', 'doc', 'post', 'history', 'deck', 'plan'] }, description: 'Narrow to a kind of source. Omit to search everything the visitor may see.' }
        },
        required: ['query']
      }
    }
  },
  {
    ...
    function: {
      name: 'get_benchmark',
      description: 'The published concurrency ladder: Metrale against the matched vLLM configuration on the same box, every rung, with the workload and the link to the results log. The only source for measured performance numbers.',
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Run the payback model from the pricing page. Three scenarios: "fleet" (get more out of GPUs you own), "api" (replace a metered API bill with boxes you own), "energy" (tokens per joule). Any input left out takes the page default. Results are modeled, not measured, and must be labelled that way.',
      parameters: {
        type: 'object',
        properties: {
          ...
          inputs: {
            type: 'object',
            description: 'Overrides. fleet: gpus, gpuCostPerYear, utilization, uplift, licensePerGpuYear, wattsPerGpu, pue, usdPerKwh, replacedSoftwarePerGpuYear. api: monthlySpend, usdPerMillionTokens, boxTokensPerSecond, boxCapex, amortMonths, utilization, wattsPerBox, pue, usdPerKwh, licensePerBoxMonth. energy: tokensPerSecond, baselineTokensPerSecond, watts, baselineWatts, pue, usdPerKwh, millionTokensPerMonth.',
  ...
  },
  {
    ...
    function: {
      name: 'list_pages',
      description: 'The map of this website: every page with its path, title and one line description. Use it to point the visitor at the right page.',
  ...
  },
  {
    ...
    function: {
      name: 'next_steps',
      description: 'What a visitor can do next, with links: a working session on their workload, a scoped pilot, the Community Edition waitlist, the deck on request, the open source engine, the contributing guide, the contact paths. Pick by who is asking.',
      parameters: { type: 'object', properties: { audience: { type: 'string', enum: ['infra', 'investor', 'contributor', 'curious'] } }, required: ['audience'] }
    }
  },
  {
    type: 'function',
    function: {
      name: 'repo_activity',
      description: 'The public repository as of the knowledge base build: a summary (stars, forks, license, contributors), the latest releases, the latest commits, the latest merged pull requests, or the contributors by commit count.',
      parameters: {
        type: 'object',
        properties: { kind: { type: 'string', enum: ['summary', 'releases', 'commits', 'pulls', 'contributors'] }, limit: { type: 'integer', minimum: 1, maximum: 40 } },
        required: ['kind']
      }
    }
  },
  {
    ...
    function: {
      name: 'capture_lead',
      description: 'Send the visitor\'s details to the Metrale team so a person follows up. Only after the visitor has given their name and email and agreed to be contacted. Never call it twice in one conversation.',
      parameters: {
        type: 'object',
        properties: {
          name: { type: 'string' },
          email: { type: 'string' },
          company: { type: 'string' },
          interest: { type: 'string', description: 'What they want: a working session, a pilot, the deck, partnership, hiring, press, something else.' },
...
function stepsFor(audience, data, site) {
  ...
  const byAudience = {
    infra: [
      { step: 'Book a working session on your workload: the console on demo data, the ladder, and the payback model with your inputs', url: p(routes.demoForm ?? '/demo#book') },
      { step: 'Scope a four week proof of value: one model, one hardware target, one workload, a side by side ladder in week one', url: p(routes.pricing ?? '/pricing') },
      { step: 'Read how it deploys: hosted, your cloud account, on premises or air gapped', url: p(routes.deployment ?? '/platform/deployment') },
      { step: 'Email sales and pilots', url: `mailto:${contacts.sales ?? ''}` }
    ],
    investor: [
      { step: 'Request the deck and the verification walkthrough from the team', url: `mailto:${contacts.press ?? ''}?subject=Metrale%20deck` },
...
export async function runTool(name, rawArgs, ctx) {
  ...
  const { index, tiers, data = {}, env = {}, site = '', sources, page = '' } = ctx;
  ...
  switch (name) {
    case 'search_site': {
      ...
      if (ctx.searches > (ctx.maxSearches ?? 4)) return { error: 'The search limit for this answer is reached. Answer from the passages you have, and say what they do not cover.' };
      const kinds = Array.isArray(args.kinds) && args.kinds.length ? args.kinds : null;
      const hits = index.search(query, { k: 6, tiers, kinds });
      if (hits.length === 0) return { results: [], note: 'Nothing matched. Try other words, or say that the published material does not cover it.' };
      const results = [];
      let budget = MAX_RESULT_CHARS;
      for (const { doc } of hits) {
        const n = cite(ctx, { id: doc.id, title: doc.title, section: doc.section ?? '', url: doc.url ?? '', kind: doc.kind, tier: doc.tier });
    ...
    case 'get_benchmark': {
      ...
      const n = cite(ctx, { id: 'tool:ladder', title: 'The concurrency ladder', section: l.box?.gpu ?? '', url: `${site}${data.routes?.benchmarks ?? '/benchmarks'}`, kind: 'page', tier: 'public' });
      return {
        cite_as: n,
        title: l.title,
        subtitle: l.subtitle,
        ...
        rows: (l.rows ?? []).map((r) => ({ concurrency: r.c, metrale_tok_s: r.atlas, matched_baseline: r.baseline, matched_baseline_tok_s: r.baseline_tok_s, ratio: r.ratio })),
        summary: l.summary,
    ...
    case 'estimate_economics': {
      ...
      const n = LABELS[scenario] ? cite(ctx, { id: `tool:payback:${scenario}`, title: 'The payback model', section: LABELS[scenario], url: pricing, kind: 'page', tier: 'public' }) : 0;
      if (scenario === 'fleet') {
        const used = { ...FLEET_DEFAULTS, ...inputs };
        return { cite_as: n, scenario, modeled: true, inputs_used: used, evidence: { uplift: 'USER, defaults below the measured GB10 ratio', licensePerGpuYear: 'PROPOSED list price', others: 'USER' }, result: fleetModel(used), page: pricing };
      }
      if (scenario === 'api') {
        const used = { ...API_DEFAULTS, ...(top ? { boxTokensPerSecond: top.atlas } : {}), ...inputs };
        return { cite_as: n, scenario, modeled: true, inputs_used: used, evidence: { boxTokensPerSecond: top ? `MEASURED at C=${top.c} on ${data.ladder.box?.gpu ?? 'the published box'}` : 'default', licensePerBoxMonth: 'PROPOSED', others: 'USER' }, result: apiModel(used), page: pricing };
      }
      if (scenario === 'energy') {
        const ladderRows = rows.map((r) => ({ c: r.c, atlas: r.atlas, best_baseline_id: 'm', baselines: [{ id: 'm', tok_s: r.baseline_tok_s }] }));
        const used = { ...ENERGY_DEFAULTS, ...energyInputsFrom(ladderRows), ...inputs };
        return { cite_as: n, scenario, modeled: true, inputs_used: used, evidence: { tokensPerSecond: 'MEASURED, top rung', baselineTokensPerSecond: 'MEASURED, same rung', watts: 'USER, the ladder publishes no power; 240 W is the DGX Spark power supply rating, a ceiling', others: 'USER' }, result: energyModel(used), page: pricing };
      }
    ...
    case 'list_pages': {
      ...
      return { pages: picked.slice(0, 40).map((p) => ({ path: p.path, url: `${site}${p.path === '/' ? '' : p.path}`, title: p.title, description: clip(p.description, 200) })) };
    }
    ...
    case 'repo_activity': {
      ...
      const n = cite(ctx, { id: `tool:repo:${kind}`, title: 'The repository on GitHub', section: `${kind}, as of ${h.as_of}`, url: kind === 'releases' ? `${repo}/releases` : kind === 'commits' ? `${repo}/commits/main` : kind === 'pulls' ? `${repo}/pulls?q=is%3Apr+is%3Amerged` : kind === 'contributors' ? `${repo}/graphs/contributors` : repo, kind: 'history', tier: 'public' });
      switch (kind) {
        case 'releases':
        ...
        case 'contributors':
          return { cite_as: n, as_of: h.as_of, contributors: (h.contributors ?? []).slice(0, limit), note: 'Commit counts. Roles are not recorded here.' };
        default:
    ...
    case 'capture_lead': {
      ...
      if (!name || !EMAIL.test(email) || !interest) return { ok: false, error: 'A name, a valid email address and what they want are needed.' };
      if (ctx.leadCaptured) return { ok: false, error: 'A lead was already sent in this conversation. Tell the visitor it is with the team.' };
      ...
      if ((ctx.turns ?? 2) < 2) return { ok: false, error: 'Not yet. Repeat the name, the email, the company and what they want back to the visitor in one line and ask them to confirm. Send it when they say yes.' };
      const lead = { source: 'prime', name, email, company, notes: `${interest}${notes ? `\n\n${notes}` : ''}`, page: clip(page, 200) };
      let id = null;
      if (env.FORMS_ENDPOINT) {
        try {
          const res = await (ctx.fetchImpl ?? fetch)(env.FORMS_ENDPOINT, { method: 'POST', headers: { 'content-type': 'application/json', origin: site }, body: JSON.stringify({ ...lead, website: '' }) });
          const body = await res.json().catch(() => ({}));
          if (!res.ok || !body.ok) return { ok: false, error: 'The team could not be reached. Give the visitor the sales address instead.' };
          id = body.id ?? null;
        } catch {
        ...
        try {
          await env.PRIME.put(`lead:${new Date().toISOString()}:${id}`, JSON.stringify({ ...lead, id, at: new Date().toISOString() }), { expirationTtl: 180 * 86400 });
        } catch {
      ...
      return { ok: true, id, note: 'Sent. Tell the visitor a person will reply, and which address to write to if they want to add anything.' };
    }
