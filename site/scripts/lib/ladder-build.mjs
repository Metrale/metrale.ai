...
function checkInstrument(s, workload) {
  ...
  for (const k of REQUIRED_INSTRUMENT) {
    if (inst[k] === undefined || inst[k] === null || inst[k] === '') fail(`baseline ${s.id} instrument.${k} is undeclared`);
  }
  ...
  if (kv !== inst.kv_cache_dtype && !(typeof s.instrument_note === 'string' && s.instrument_note.trim()))
    fail(
      `baseline ${s.id} instrument.kv_cache_dtype=${inst.kv_cache_dtype} but its cli says ${kv ?? 'nothing'}; add instrument_note saying why`
    );
}
...
function checkUnmeasured(s, docs) {
  ...
  for (const c of u.rungs) {
    ...
    for (const [file, doc] of docs) {
      if ((doc.rungs ?? []).some((r) => r.concurrency === c)) fail(`series ${s.id} lists C=${c} as unmeasured but ${file} contains it`);
    }
  }
}
...
export function buildLadder(manifest, { subject, rawOf, harnessRepoSha256 }) {
  ...
  if (matchedAll.length > 1)
    fail(
      `more than one full-ladder matched-parity baseline (${matchedAll.map((s) => s.id).join(', ')}) — exactly one leg is the published ratio's reference`
    );
