...
export function mergeSmallSiblings(sections, { max = 1600 } = {}) {
  ...
  for (const s of sections) {
    ...
    const fits =
      last &&
      s.level === 3 &&
      (last.level === 2 || last.merged) &&
      last.trail?.[0] === s.trail?.[0] &&
      parentText.length + childText.length + s.heading.length < max;
    if (fits) {
...
export function chunkText(text, { max = 1400, min = 80 } = {}) {
  ...
  for (const c of out) {
    if (
      folded.length &&
      (c.length < min || folded[folded.length - 1].length < min) &&
      folded[folded.length - 1].length + c.length + 1 <= max
    )
