...
test('text is escaped before any markup is made', () => {
  expect(renderMarkdown('<script>alert(1)</script> & "quotes"')).toBe(
...
test('links: external ones open in a new tab, site paths and anchors stay in the page', () => {
  expect(renderMarkdown('See [pricing](/pricing#payback) and [the repo](https://github.com/x/y) and [up](#top).')).toBe(
    '<p>See <a href="/pricing#payback">pricing</a> and <a href="https://github.com/x/y" rel="noopener nofollow" target="_blank">the repo</a> and <a href="#top">up</a>.</p>'
  );
...
test('citations become marked superscripts and never eat a link', () => {
  expect(renderMarkdown('Eight wins [1]. Two sources [2, 3] and [4][5].')).toBe(
    '<p>Eight wins <sup class="pr-cite" data-n="1">[1]</sup>. Two sources <sup class="pr-cite" data-n="2">[2]</sup><sup class="pr-cite" data-n="3">[3]</sup> and <sup class="pr-cite" data-n="4">[4]</sup><sup class="pr-cite" data-n="5">[5]</sup>.</p>'
  );
...
test('blocks: headings step down a rank, lists nest once, quotes, rules, fences', () => {
  ...
  expect(renderMarkdown(md)).toBe(
    '<h2>Top</h2><h3>Second</h3><p>Para</p><ul><li>one</li><li>two<ul><li>two b</li></ul></li></ul><ol><li>first</li><li>second</li></ol><blockquote>quoted</blockquote><hr><pre class="pr-fence"><code class="language-js">const x = &quot;&lt;b&gt;&quot;;</code></pre>'
  );
});
...
test('a table renders with a header row, and a wrapped list item stays one item', () => {
  ...
  expect(renderMarkdown(md)).toBe(
    '<div class="pr-table"><table><thead><tr><th>C</th><th>Metrale</th><th>vLLM</th></tr></thead><tbody><tr><td>1</td><td>23.6</td><td>19.7</td></tr><tr><td>128</td><td>478.1</td><td>358.6</td></tr></tbody></table></div>'
  );
