...
>
  ...
  <header class="pr-head">
    ...
    <div class="pr-head-tools">
      <PrimeTelemetry />
      {#if prime.messages.length}<button type="button" class="pr-icon" title={copy.reset} aria-label={copy.reset} onclick={reset}
          ><svg
            width="16"
            height="16"
            viewBox="0 0 24 24"
            ...
            aria-hidden="true"><path d="M3 12a9 9 0 1 0 3-6.7L3 8" /><path d="M3 3v5h5" /></svg
          ></button
        >{/if}
      <button type="button" class="pr-icon" aria-label={copy.close} onclick={onclose}
        ><svg
          width="16"
          height="16"
          viewBox="0 0 24 24"
      ...
      <div class="pr-welcome">
        ...
        <div class="pr-aud" role="group" aria-label="Who is asking">
          {#each copy.audiences as a (a.id)}
            <button type="button" class="pr-aud-chip" aria-pressed={prime.audience === a.id} onclick={() => setAudience(a.id)}
              >{a.label}</button
            >
          {/each}
        </div>
  ...
  >
    {#if prime.messages.length > 0 && prime.audience}
      <p class="pr-composer-aud">
        {audience?.label}<button
          type="button"
          class="pr-composer-aud-x"
          onclick={() => setAudience(prime.audience)}
          aria-label="Clear who is asking">✕</button
        >
      </p>
    {/if}
    <div class="pr-composer-row">
      ...
      {#if busy}
        <button type="button" class="pr-send is-stop" onclick={stop} aria-label={copy.composer.stop}
          ><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"
            ><rect x="5" y="5" width="14" height="14" rx="2" /></svg
          ></button
        >
      {:else}
        <button type="submit" class="pr-send" disabled={!draft.trim()} aria-label={copy.composer.send}
          ><svg
            width="16"
            height="16"
            viewBox="0 0 24 24"
    ...
    <div class="pr-foot">
      ...
      <button type="button" class="pr-code-btn" aria-expanded={codeOpen} onclick={() => (codeOpen = !codeOpen)}
        >{prime.access ? copy.partner.on : copy.partner.label}</button
      >
    </div>
    {#if codeOpen}
      <div class="pr-code" role="group" aria-label={copy.partner.group}>
        ...
        {:else}
          <input
            ...
            placeholder={copy.partner.placeholder}
            aria-label={copy.partner.label}
            autocomplete="off"
            onkeydown={(e) => e.key === 'Enter' && applyCode(e)}
          />
...
<style>
  ...
  .pr-aud-chip:focus-visible,
  .pr-starter:focus-visible {
    outline: 2px solid var(--accent);
    outline-offset: 2px;
  }
  .pr-starters {
  ...
  .pr-code-btn:hover,
  .pr-code-btn[aria-expanded='true'] {
    color: var(--accent);
  }
  .pr-code {
    display: flex;
