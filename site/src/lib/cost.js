...
export function savingsOverTime({ metraleJPerTok, baseJPerTok, tokensPerDay, usdPerKwh, pue, days }) {
  ...
  if (!Number.isFinite(tokensPerDay) || tokensPerDay <= 0)
    return { state: 'unavailable', why: 'daily token demand must be a positive number' };
...
export function readEnergy(m, prefix, run, id, expectTokS) {
  ...
  if (energyJ <= 0) return refuse(`${prefix}${KEY.energyJ} is ${energyJ} — a window cannot cost nothing (or less)`);
...
const pointsOfSeries = (series) => (series.rungs ?? []).map((r) => ({ c: r.c, energy: energyOfRung(series.label ?? series.id, r) }));
...
export function idleAvailability(metrale, baselines) {
  ...
  const missing = sides.filter((s) => !s.points.some((p) => p.energy.state === 'measured' && p.energy.idleW !== null)).map((s) => s.label);
  return missing.length === 0 ? { both: true, why: '' } : { both: false, why: `idle not recorded for ${missing.join(', ')}` };
}
...
export function costTrend(c, records) {
  ...
  gens.forEach((g, i) => {
    ...
    g.verdict = spreadVerdict(
      g.members.map((u) => u.e.tokPerWh),
      g.spread
    );
...
const summariseDiffers = (differs, show = 3) =>
  differs.length <= show ? describeDiffers(differs) : `${describeDiffers(differs.slice(0, show))}, +${differs.length - show} more axes`;
    ...
    series.length === 0
      ...
      : series
          .map((b) => {
            ...
            const cmp = against ? comparable(against, { checkpoint: ladder.workload.checkpoint, instrument: b.instrument }) : null;
            const where =
              cmp === null
