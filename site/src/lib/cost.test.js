...
describe('absent is not zero', () => {
  ...
  test('an absent cell is skipped by the verdicts and by the trend, not counted as 0', () => {
    ...
    const v = rungVerdicts(
      [{ c: 16, energy: energyOf(r, 16) }],
...
describe('a losing rung is returned like any other, and named', () => {
  ...
  test('k = 0 flips the headline tile to name vLLM', () => {
    const v = rungVerdicts([metraleAt(1, 80), metraleAt(8, 80)], [{ label: 'vLLM', points: [rivalAt(1, 40, 20), rivalAt(8, 40, 20)] }]);
    expect(v.k).toBe(0);
    expect(v.n).toBe(2);
  ...
  test('a mixed result counts only the rungs Metrale Engine actually wins', () => {
    const v = rungVerdicts([metraleAt(1, 80), metraleAt(8, 20)], [{ label: 'vLLM', points: [rivalAt(1, 40, 20), rivalAt(8, 40, 20)] }]);
...
describe('the empty state', () => {
  ...
  test('a vLLM series ON the gate instrument says that instead', () => {
    const e = emptyStateOf(SUBJECT, [rec({ metrics: { c8_aggregate_tok_s: 1 } })], laddersWith([baselineSeries([{ c: 8, tok_s: 20 }])]));
...
const otherInstrument = (c4) =>
  rec({ params: { concurrencies: '1, 4, 8, 16', isls: '512', osl: '320', prompt_mode: 'natural' }, metrics: { c4_aggregate_tok_s: c4 } });
...
// ---------------------------------------------------------------------------
// The sampler-coverage guard runs on every record, or says it could not
//
...
describe('PUE, the second reader input', () => {
  test('the default applies NOTHING — it is the identity, not a guess', () => {
    ...
    for (const j of [3.2, 0.13, 1.9]) {
      expect(costPerMillion(j, DEFAULT_USD_PER_KWH, DEFAULT_PUE)).toBe((j * DEFAULT_USD_PER_KWH) / 3.6);
    }
  });
  ...
  test('isPue refuses below 1.0 — a building cannot use less than its IT load', () => {
    ...
    for (const v of [0.999, 0, -1, PUE_MAX + 0.001, NaN, Infinity, null, undefined, '1.2']) expect(isPue(v)).toBe(false);
  });
