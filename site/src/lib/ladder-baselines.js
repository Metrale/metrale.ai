...
// been re-pointed onto the published axes, so the two DO pair now --
 ...
 */
export const REQUIRED_AXES = Object.freeze(['isl', 'osl', 'prompt_mode', 'max_model_len', 'max_batch_size', 'kv_cache_dtype']);
...
export function instrumentOf(x) {
  ...
  if (isGate === isBaseline) {
    throw new TypeError(
      `instrumentOf: expected a gate record (params) or a baseline (instrument), got ${JSON.stringify(x && Object.keys(x))}`
    );
  }
...
export function comparable(a, b) {
  ...
  const extras = [...new Set([...Object.keys(A.axes), ...Object.keys(B.axes)])].filter((axis) => !required.has(axis)).sort();
