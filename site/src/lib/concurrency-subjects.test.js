...
describe('recordsOf', () => {
  const recordsFor = store({
    'concurrency-sweep': [
      rec('concurrency-sweep', DENSE, 'd1'),
      rec('concurrency-sweep', MOE, 'stray'),
      rec('concurrency-sweep', DENSE, 'd2'),
    ],
...
describe('parseConcurrencies', () => {
  ...
  test.each(['', '1, , 4', '1, 2, four', '0, 1', '-4', '1.5', undefined, null])('refuses %p loudly, naming the record', (bad) => {
...
describe('unassignedRecords', () => {
  test('lists every unclaimed bench · model with its count and nothing that is claimed', () => {
    ...
    const recordsFor = store({
      'concurrency-sweep': [
        rec('concurrency-sweep', DENSE, 'a'),
        rec('concurrency-sweep', stray, 'b'),
        rec('concurrency-sweep', stray, 'c'),
      ],
    ...
    expect(out.map(formatUnassigned)).toEqual([`concurrency-sweep · ${stray} (2)`, `concurrency-sweep-dflash2 · ${MOE} (1)`]);
  });
