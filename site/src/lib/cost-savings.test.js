...
// baseline 2.0 J/tok, Metrale Engine 1.2 J/tok, 1e9 tokens/day, $0.15/kWh, PUE 1.0.
//   saved per day = (2.0 - 1.2) * 1e9 / 3.6e6 kWh = 222.22... kWh
//   at $0.15                                      = $33.33...
const base = { metraleJPerTok: 1.2, baseJPerTok: 2.0, tokensPerDay: 1e9, usdPerKwh: 0.15, pue: 1.0, days: 30 };
...
describe('savingsOverTime', () => {
  ...
  test('it agrees with costPerMillion — the two must not drift apart', () => {
    // Savings per day must equal (baseline $/M − Metrale Engine $/M) × millions per day.
    const perM = (j) => costPerMillion(j, base.usdPerKwh, base.pue);
    const expected = (perM(base.baseJPerTok) - perM(base.metraleJPerTok)) * (base.tokensPerDay / 1e6);
  ...
  test('a LOSS is reported as a loss, not clamped to zero', () => {
    // The whole point of the guard: if Metrale Engine spends more per token, say so.
    const r = savingsOverTime({ ...base, metraleJPerTok: 2.5 });
    expect(r.state).toBe('measured');
  ...
  test('every missing input is refused BY NAME, not as a generic failure', () => {
    ...
    expect(why({ metraleJPerTok: null }).why).toContain('Metrale Engine energy');
    ...
    for (const over of [{ metraleJPerTok: null }, { days: 0 }]) expect(why(over).state).toBe('unavailable');
  });
