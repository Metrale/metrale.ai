<script>
  ...
  const values = $derived(seriesList.flatMap((s) => measured(s).flatMap((p) => [usdAt(p.energy, pue.low), usdAt(p.energy, pue.high)])));
  ...
  const bandOf = (s) => {
    ...
    const out = pts.map((p, i) => `${i ? 'L' : 'M'}${x(p.c).toFixed(1)} ${y(usdAt(p.energy, pue.high)).toFixed(1)}`).join(' ');
    const back = [...pts]
      .reverse()
  ...
  const metraleAbsent = $derived(cost.metrale ? cost.metrale.points.filter((p) => p.energy.state !== 'measured') : []);
  ...
  const loseAnchor = (c) => (x(c) - LOSE_HALF_W < PL ? 'start' : x(c) + LOSE_HALF_W > W - PR ? 'end' : 'middle');
  ...
  const tip = (label, p) => {
    ...
    const parts = [
      `${label} · C=${p.c}`,
      pue.band
        ? `$${fmtUsd(usdAt(e, pue.low))}–${fmtUsd(usdAt(e, pue.high))} per 1M tokens at ` + `${usdPerKwh} $/kWh, PUE ${pue.low}–${pue.high}`
...
<figure class="gate-panel cost-chart">
  ...
  <svg
    ...
    aria-label="Cost per million tokens against concurrency for {subject.label}, GPU rail only, {cost.baselines.length
      ...
      ? `, shaded between PUE ${pue.low} and ${pue.high}`
      : pue.low === 1
        ? ''
        : `, at PUE ${pue.low}`}"
  >
    ...
    <text class="cost-rail" x={PL} y={PT - 12}> GPU rail only · Grace CPU and LPDDR5X are outside these numbers · lower bound </text>
    ...
    {#if pue.band}
      {#each seriesList as s}
        {@const d = bandOf(s)}
        {#if d}
          <path class="cost-band" {d} style="--band: {s.role === 'metrale' ? color : 'var(--t2)'}" />
        {/if}
        {#each ticksOf(s) as p}
          <line
            class="cost-band-tick"
            x1={x(p.c)}
            y1={y(usdAt(p.energy, pue.low))}
            x2={x(p.c)}
            y2={y(usdAt(p.energy, pue.high))}
      ...
      {#each measured(b) as p}
        ...
        <g>
          ...
          <rect
            ...
            width="7"
            height="7"
            fill={p.energy.trusted ? 'var(--t2)' : 'none'}
            stroke="var(--t2)"
            stroke-width="1.5"
          />
        </g>
    ...
    {#if cost.metrale && metraleMeasured.size}
      ...
      {#each measured(cost.metrale) as p}
        ...
        <g
          ...
          role="button"
          tabindex="0"
          aria-label="C={p.c}: ${fmtUsd(usdOf(p.energy))} per million tokens{v?.loseLabel ? `, ${v.loseLabel}` : ''}{p.energy.trusted
            ? ''
            : ', under-sampled'} — details"
          onclick={() => cost.metrale.record && onselect([cost.metrale.record])}
          onkeydown={(e) =>
            (e.key === 'Enter' || e.key === ' ') && (e.preventDefault(), cost.metrale.record && onselect([cost.metrale.record]))}
        >
          <title
            >{tip(cost.metrale.label, p)}{v
              ? ` · ${v.metraleCheaper ? `Metrale Engine cheaper ×${(1 / v.ratio).toFixed(2)}` : v.loseLabel} vs ${v.rivalLabel}`
              : ''}</title
          >
          ...
          <circle
            ...
            cy={py}
            r="3.5"
            fill={p.energy.trusted ? color : 'var(--card)'}
            stroke={color}
            stroke-width="1.8"
          />
        </g>
        ...
        {#if v?.loseLabel}
          <text class="cost-lose" x={loseX(p.c)} y={Math.min(py + 15, H - PB - 2)} text-anchor={loseAnchor(p.c)}>{v.loseLabel}</text>
        {/if}
      {/each}
    {/if}
  </svg>
