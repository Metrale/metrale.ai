<script module>
  ...
  import {
    ...
    batchCapOf as _batchCapOf,
    comparisonStateOf as _comparisonStateOf,
    instrumentLabel as _instrumentLabel,
    ladderFor,
    liveRecordOf as _liveRecordOf,
  ...
  // Re-exported for the panel and the tests; bound here so the instance
  // script below reads the same functions.
  export const batchCapOf = _batchCapOf;
  export const instrumentLabel = _instrumentLabel;
  export const liveRecordOf = _liveRecordOf;
  export const measuredRange = _measuredRange;
</script>
  ...
  <figure class="gate-panel cmp">
    ...
    <div class="cmp-keys" role="group" aria-label="Series drawn">
      <button
        ...
        aria-pressed={shown(metrale.id)}
        onclick={() => toggle(metrale.id)}>Metrale Engine · published campaign · {measuredRange(metrale.rungs)}</button
      >
      {#each baselines as b}
        <button type="button" class="cmp-chip" aria-pressed={shown(b.id)} onclick={() => toggle(b.id)}>{oneShotChip(b)}</button>
      {/each}
    </div>
    {#if refused}
...
{:else}
  <figure class="gate-panel cmp">
        ...
        <span class="gate-legend">
          ...
          {#if floorPath || hasViol}
            ...
            <span class="gl-group">
              ...
              {#if hasViol}
                <span class="gate-legend-item">
                  <svg class="gl-swatch" viewBox="0 0 12 10" aria-hidden="true"><circle class="gc-viol" cx="6" cy="5" r="4.2" /></svg>
                  below its floor
                </span>
              {/if}
            </span>
    ...
    >
      ...
      {/if}
      {#if pts.length}
        <path d={path} fill="none" stroke={color} stroke-width="2" stroke-dasharray={dash} stroke-linejoin="round" stroke-linecap="round" />
        {#each pts as p}
          ...
          >
            <title
              >Metrale Engine · C={p.c} · {fmtV(p.v)} tok/s{broke} · gate record {fmtDate(live.recorded_at)} · {live.git_sha} · click for record</title
            >
    ...
    {:else if state === 'live'}
      <p class="cmp-caption">
        ...
        {#each pair.refused as r}
          The {r.series.label} one-shot of {measuredRange(r.series.rungs)} is on another instrument and is not comparable: {r.why}.
        {/each}
        {#if pair.refused.length === 0 && refIsSameCheckpoint}
          The published ladder's vLLM legs are ISL {ref.workload.isl_tokens} / OSL {ref.workload.osl_tokens}{refCap
            ? ` at batch cap ${refCap}`
            : ''}
    ...
    {:else}
      ...
      <div class="cmp-empty">
        ...
        <p>
          ...
          <code>status = "unmeasured"</code> and no metrics table — thresholds follow the first run on main, by design.
        </p>
        ...
        {#if /fp8/i.test(subject.checkpoint)}
          <p>
            Note: the chart prints the checkpoint id, which is what a record carries — an FP8 checkpoint here, whatever a <code>quant</code> line
            elsewhere says.
          </p>
        {/if}
        {#if subject.published_manifest !== null}
          <p>
            Note: <code>{subject.published_manifest}</code> is declared for this subject but the site has no generated ladder for it; run
            <code>node site/scripts/gen-ladder.mjs</code>.
          </p>
        {/if}
      </div>
    {/if}
