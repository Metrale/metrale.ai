    ...
    <nav class="av-nav" aria-label="Main">
      <ul>
        {#each nav.groups as g}
          {#if g.columns}
            <li class="av-nav-item" class:is-open={open === g.label} class:is-broad={g.columns.length > 2} onpointerenter={() => enter(g.label)} onpointerleave={leave}>
              <button
                type="button"
                class="av-nav-btn"
...
<style>
  ...
  .av-nav-item.is-broad { position: static; }
  .av-nav-item.is-broad .av-mega { top: calc(100% - 4px); }
  .av-mega-in {
  ...
  .av-mega-in.is-wide { grid-template-columns: repeat(var(--cols, 2), minmax(0, 1fr)); min-width: min(calc(var(--cols, 2) * 320px), calc(100vw - 32px)); }
  ...
  .av-cta {
    display: inline-flex; align-items: center; padding: 0.6rem 1.1rem; border-radius: 999px; background: var(--accent-fill); color: var(--on-accent);
  ...
  .av-drawer-col { margin: 0.6rem 0 0.1rem; padding: 0 0.9rem; font-family: var(--font-mono); font-size: 0.66rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--t3); font-weight: 600; }
  .av-drawer-col:first-child { margin-top: 0.2rem; }
