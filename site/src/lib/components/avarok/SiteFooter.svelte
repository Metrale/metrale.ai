  ...
  <div class="av-footer-in">
    <div class="av-footer-brand">
      ...
      <div class="av-footer-social">
        <a href={links.github} aria-label="Metrale on GitHub" target="_blank" rel="noopener">
        ...
        <a href={links.discord} aria-label="Metrale on Discord" target="_blank" rel="noopener">
      ...
      <div class="av-footer-col">
        ...
        {#each col.links as l}
          <a href={l.href} target={l.external ? '_blank' : undefined} rel={l.external ? 'noopener' : undefined}
            >{l.text}{#if l.external}<span aria-hidden="true"> ↗</span>{/if}</a
          >
        {/each}
      </div>
    {/each}
...
<style>
  ...
  .av-footer-brand > a:first-child {
    display: inline-block;
    text-decoration: none;
    color: inherit;
  }
  .av-footer-brand :global(.logo) {
    margin: 0;
  }
  .av-footer-slogan {
