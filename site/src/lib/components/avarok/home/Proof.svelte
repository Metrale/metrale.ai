  ...
  <div class="av-container">
    <div class="av-split av-split-wide">
      ...
      <div class="av-card av-reveal av-ladder-card">
        ...
        <svg viewBox={`0 0 ${W} ${H}`} class="av-ladder" role="img" aria-label={`Aggregate tokens per second, Metrale against the matched vLLM configuration, from C=1 to C=${maxC}`}>
          {#each ticks as t}
          ...
          {/each}
          <text x={W - PR} y={y(rows.at(-1).atlas) - 10} text-anchor="end" class="lab atlas-lab">Metrale {rows.at(-1).atlas.toFixed(0)} tok/s</text>
