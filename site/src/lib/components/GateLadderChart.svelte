<script>
  ...
  const hasViol = $derived(lanes.some((l) => [l.previous, l.latest].filter(Boolean).some((r) => r.pts.some((p) => violAt(r, p)))));
        ...
        <g opacity={isLatest ? 1 : 0.45}>
          ...
          {#each r.pts as p}
            ...
            <g
              ...
              role="button"
              tabindex="0"
              aria-label="C={p.c}: {fmtV(p.v)} tok/s{broke}{lane.variant ? ', ' + lane.variant : ''} on {fmtDate(r.rec.recorded_at)}, {r.rec
                .verdict} — details"
              onclick={() => onselect([r.rec])}
              onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (e.preventDefault(), onselect([r.rec]))}
            >
              <title
                >C={p.c} · {fmtV(p.v)} tok/s{broke}{lane.variant ? ' · ' + lane.variant : ''} · {fmtDate(r.rec.recorded_at)} · {r.rec
                  .verdict} · click for record</title
              >
