...
<figure class="topo-fig">
  {#if ordered.length === 0}
    ...
    <div class="topo-empty">
      ...
      <p class="topo-empty-sub">Two of them, on the same network, are what the EP=2 recipes need.</p>
    </div>
  {:else}
    <svg class="topo-svg" viewBox="0 0 {W} {H}" role="img" aria-label={label}>
      ...
      {#each edges as e (e.a.node.id + e.b.node.id)}
        <line in:scale={{ duration: 260 }} class="topo-edge" class:topo-edge-warn={e.warn} x1={e.a.x} y1={e.a.y} x2={e.b.x} y2={e.b.y} />
        <text class="topo-edge-label" x={(e.a.x + e.b.x) / 2} y={(e.a.y + e.b.y) / 2 - 10} text-anchor="middle">
          {linkText(e.cls)}{#if e.speed}
            {Math.round(e.speed / 1000)}G{/if}
        </text>
      {/each}
