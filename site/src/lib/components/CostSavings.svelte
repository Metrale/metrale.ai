<script>
  ...
  import { savingsOverTime, SAVINGS_HORIZONS, DEFAULT_TOKENS_PER_DAY, TOKENS_PER_DAY_STORAGE_KEY, fmtUsd } from '$lib/cost.js';
  ...
  function onTokens(e) {
    ...
    tokensInput = v;
    try {
      if (Number.isFinite(v) && v > 0) localStorage.setItem(TOKENS_PER_DAY_STORAGE_KEY, String(v));
    } catch {
      /* ignore */
    }
  }
...
<section class="cost-savings" aria-labelledby="cost-savings-h">
  ...
  {:else}
    <div class="cost-savings-inputs">
      <label>
        tokens per day
        <input type="number" min="1" step="1e8" value={tokensInput} oninput={onTokens} aria-describedby="cost-savings-h" />
      </label>
      <div class="cost-savings-horizons" role="group" aria-label="horizon">
        {#each SAVINGS_HORIZONS as h}
          <button
            type="button"
            class="cost-horizon"
            class:is-on={days === h.days}
            aria-pressed={days === h.days}
            onclick={() => (days = h.days)}>{h.label}</button
          >
        {/each}
      </div>
    </div>
