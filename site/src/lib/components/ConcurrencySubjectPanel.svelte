<script>
  ...
  // "how are we moving".
  //
  // The two charts share ONE instrument as of 2026-09-21/22: the
  ...
  const gatePanel = $derived(latest ? { title: `latest gate sweep · ${instrumentLabel(latest)}`, unit: 'tok/s' } : null);
</script>

<div class="csp" id="cs-panel-{subject.id}" role="tabpanel" aria-labelledby="cs-tab-{subject.id}" tabindex="-1">
  ...
  {#if latest && gatePanel}
    <p class="cmp-bridge">
      {#if state === 'published'}
        ...
        <code>{instrumentLabel(latest)}</code>, against the published pair's ISL {published.workload.isl_tokens} / OSL {published.workload
          .osl_tokens} with speculation pinned — so its tok/s are not the ladder's tok/s. Read each chart against its own history, never one against
        the other.
      {:else if live}
        Same instrument as the chart above, which draws the newest passing run on main ({fmtDate(live.recorded_at)} · {live.git_sha}); this
        chart adds the run history around it.
      {:else}
        No passing run on main yet to compare against; the runs here are from branches or did not pass.
      {/if}
    </p>
