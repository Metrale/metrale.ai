<script>
  ...
  const usdAt = (e) =>
    fmtUsd(costPerMillion((showAboveIdle && e.aboveIdleJ !== null ? e.aboveIdleJ : e.energyJ) / e.tokens, usdPerKwh, pue.low));
  ...
  const chartTitle = $derived(
    `$ per 1M tokens · ${latest ? instrumentLabel(latest) : subject.gate} · ${showAboveIdle ? 'above idle (marginal)' : 'total'}`
  );
  ...
  const selectedRung = $derived(typeof rung === 'number' && rungsMeasured.includes(rung) ? rung : defaultRung);
...
<div class="csp cost-panel" id="co-panel-{subject.id}" role="tabpanel" aria-labelledby="co-tab-{subject.id}" tabindex="-1">
  ...
  <div class="cost-price">
      ...
      <label class="cost-price-field">
        ...
        <span class="cost-price-box">
          ...
          <input
            ...
            aria-label="Electricity price in dollars per kilowatt-hour"
          />
      ...
      <div class="cost-price-field" role="group" aria-labelledby="pue-label-{subject.id}">
        ...
        <span class="cost-price-box cost-pue-box">
          <input
            ...
            step="0.05"
            inputmode="decimal"
            bind:value={pueLowInput}
            aria-label="Lowest power usage effectiveness to draw"
          />
          <span aria-hidden="true">–</span>
          <input
            type="number"
            ...
            step="0.05"
            inputmode="decimal"
            bind:value={pueHighInput}
            aria-label="Highest power usage effectiveness to draw"
          />
          <span aria-hidden="true">×</span>
        </span>
        <span class="cost-pue-presets">
          <button type="button" class="cmp-chip" aria-pressed={!pue.band && pue.low === DEFAULT_PUE} onclick={pueNone}
            >none ({DEFAULT_PUE}×)</button
          >
          <button
            type="button"
            class="cmp-chip"
    ...
    <p class="cost-price-note">
      <strong>Your input, not a measurement.</strong> The default {DEFAULT_USD_PER_KWH} $/kWh is a round retail-commercial figure — change it
      to your own tariff. It is kept in this browser and is never written into the link.
      {#if !priceValid}
        <span class="cost-warn">That is not a price; the chart is drawn at {DEFAULT_USD_PER_KWH} $/kWh until it is one.</span>
      {/if}
    </p>
    <p class="cost-price-note">
      <strong>PUE starts at {DEFAULT_PUE}×, which applies nothing.</strong> Set a low and a high — a datacentre is typically {PUE_TYPICAL.low}–{PUE_TYPICAL.high}×
      — and the chart shades the band between them for <em>both</em> engines. Because it multiplies both equally it moves every absolute
      figure and changes no ratio, no verdict and no winner.
      {#if pue.band}
  ...
  {:else}
    <div class="cost-toggles">
      <button type="button" class="cmp-chip" aria-pressed={showAboveIdle} disabled={!idle.both} onclick={() => (aboveIdle = !aboveIdle)}>
        above idle (marginal)
      </button>
    ...
    <CostLadderChart {subject} {cost} {usdPerKwh} {pue} rungs={rungsMeasured} title={chartTitle} aboveIdle={showAboveIdle} {onselect} />

    <p class="cmp-caption">
      Per-token cost is throughput divided by power, so Metrale Engine is cheaper only where its throughput lead exceeds its power cost. The
      tiles above count the rungs where it does; where it does not, the chart says <code>vLLM cheaper ×r</code> under the point and the
      count flips to name vLLM.
  ...
  {#if trend && trend.metrics.length > 0}
    ...
    <GateChart records={trend.records} panel={{ title: trend.title, unit: trend.unit, metrics: trend.metrics }} {onselect} />
    <p class="cmp-caption">
      One point is one certified run: <code>tokens ÷ joules × 3600</code> for that rung's batch. This line shows the CURRENT cost instrument
      only — {`${trend.runs} ${trend.runs === 1 ? 'run' : 'runs'}`}.
      ...
      {#if trend.superseded}
        {trend.superseded.runs} earlier {trend.superseded.runs === 1 ? 'run is' : 'runs are'} not drawn: measured on a different instrument ({trend
          .superseded.differs}), so their tok/Wh is a different measurement rather than an earlier value of this one. They stay in
      ...
      {#if newest?.spread}
        The bar on each point is the measured run-to-run spread of this rung on this instrument: across the {newest.spread.n} certified runs that
        share it, C={selectedRung} throughput spanned
        {newest.spread.tMin.toFixed(2)}–{newest.spread.tMax.toFixed(2)} tok/s. It is measured on throughput because only {newest.spread
          .powerMeasured} of those runs carry joules, and widened by the spread of the rail across those — so it is a <strong>floor</strong>
        on this line's spread, not a confidence interval.
      ...
      {:else}
        The run-to-run spread of C={selectedRung} on this instrument is not yet measured ({trend.runs}
        {trend.runs === 1 ? 'run' : 'runs'}), so this line cannot be told from noise.
      {/if}
      {#each snapshots as s}
        {s.label} measured {s.tokPerWh.toFixed(0)} tok/Wh at C={selectedRung} on {s.date} — a one-shot, not re-run, and not a line.
      {/each}
  ...
  {:else if cost.energyState !== 'none'}
    <p class="cmp-caption">
      ...
      {selectedRung === null ? 'any rung' : `C=${selectedRung}`}: the trend needs a certified run whose energy window is fully sampled.
      {#each trend?.excluded ?? [] as e}
        {e.reason}.
      {/each}
    </p>
  ...
  {#if cost.refusals.length > 0}
    <p class="bd-footnote cost-refused">
      {#each cost.refusals as r}refused: {r}.
      {/each}
    </p>
  {/if}
