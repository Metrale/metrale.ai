<script>
  ...
  import { chat, ensureReady, abortLoad, setKey, clearKey, ask, setChatModel, resetChatModel } from '../chat/state.svelte.js';
  ...
  const sourcesOpen = typeof window === 'undefined' ? true : !window.matchMedia('(max-width: 860px)').matches;
  ...
  const resetLabel = $derived(
    askError?.resetAt ? new Date(askError.resetAt).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) : ''
  );
  const FREE = ':free';
  const paidModel = $derived(chat.chatModel.endsWith(FREE) ? chat.chatModel.slice(0, -FREE.length) : '');
  ...
  const streamLive = $derived(asking && chat.stream && (chat.stream.reasoningText || chat.stream.answerText) ? chat.stream : null);
  ...
  const rendered = $derived(streamLive ? [...messages, { id: seq, role: 'assistant', live: streamLive }] : messages);

  const mb = (bytes) => (bytes / 1048576).toFixed(1);

  const statusTone = $derived(
    chat.status === 'error' ? 'error' : ready ? (asking ? 'working' : 'ready') : chat.status === 'idle' ? 'idle' : 'working'
  );
  ...
  const stages = $derived.by(() => {
    const cur = ['loading-cached', 'downloading', 'caching'].includes(chat.status) ? 'corpus' : chat.status;
  ...
  function stageDetail() {
    ...
    if (chat.status === 'indexing' || chat.status === 'loading-cached') return p.totalPoints > 0 ? `${p.indexed} of ${p.totalPoints}` : '';
    return '';
  }
  ...
  function onDialogKeydown(e) {
    ...
    const els = [
      ...dialogEl.querySelectorAll('a[href], button:not([disabled]), input:not([disabled]), summary, [tabindex]:not([tabindex="-1"])'),
      ...
      <div class="cc-panel cc-load" aria-busy="true">
        ...
        {#if chat.corpus || chat.progress.totalBytes > 0}
          <dl class="cc-load-meta">
            {#if chat.corpus}
              <div>
                <dt>{codeChat.loader.commitLabel}</dt>
                <dd>{shortCommit}</dd>
              </div>
              <div>
                <dt>{codeChat.loader.chunksLabel}</dt>
                <dd>{chat.corpus.chunks}</dd>
              </div>
            {/if}
            {#if chat.progress.totalBytes > 0}
              <div>
                <dt>{codeChat.loader.sizeLabel}</dt>
                <dd>{mb(chat.progress.totalBytes)} MB</dd>
              </div>
            {/if}
          </dl>
        {/if}
