  ...
  onMount(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        ...
        io.disconnect();
        import('./EnergyIcon.svelte').then((m) => (Icon = m.default)).catch(() => {}); // decorative: a failed chunk is simply no icon
      },
      { rootMargin }
    );
...
<span bind:this={host} class="lazy-icon" style="width:{size}px;height:{size}px" aria-hidden="true">
