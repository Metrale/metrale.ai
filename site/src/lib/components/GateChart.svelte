<script>
  ...
  const labelY = (bound, py) => (bound === 'min' ? Math.min(py + 11, H - PB - 2) : Math.max(py - 4, PT + 8));
  ...
  const describe = (s, n) => {
    ...
    const broke =
      viol === 'floor' ? ` · below floor ${fmtLimit(limit.min)}` : viol === 'ceiling' ? ` · over ceiling ${fmtLimit(limit.max)}` : '';
