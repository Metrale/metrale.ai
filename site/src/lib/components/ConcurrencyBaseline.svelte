...
<figure class="gate-panel cmp">
  ...
  <p class="cmp-caption">
    ...
    {#each engines as e, i}{i ? '; ' : ''}<code>{e}</code>{/each} on {ladder.box.name}, at this ladder's own instrument — ISL {w.isl_tokens} /
    OSL {w.osl_tokens}, {w.reps} timed reps +
    {w.warmup} warmup, temperature {w.temperature}, seed {w.seed} — and is not re-measured when Metrale Engine moves.
    {#each overridden as b}
      <strong>{b.label} is not running vLLM's default kernel here.</strong>
      {b.kernel_override.what}
      {b.kernel_override.why}
  ...
  <div class="cmp-empty">
    ...
    <p>
      What fills the comparison: one Metrale Engine leg of <code>{w.harness}</code> at these settings, filed as a series with role
      <code>subject</code>
      in <code>{ladder.manifest}</code>, whose sources name its raw files. A <code>{subject.gate}</code> gate run on main appears as the live
      series below, on the gate's own instrument; it is drawn against this one-shot only if its instrument fingerprint matches, and the ladder
      harness declares no prompt fixture where the gate does, so the chart will name the axes that differ rather than draw them on one axis.
    </p>
  </div>
