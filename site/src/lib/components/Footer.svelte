<script>
  import { footer, contactEmails, discordUrl, xUrl, nvidiaInceptionUrl } from '$lib/data.js';
...
<footer>
  <div class="footer-inner">
    <div class="footer-brand">
      <div class="flogo"><AtlasLockup kind="wordmark" /></div>
      <p>{footer.tagline}</p>
      <p class="lic">{footer.license}</p>
      <div class="footer-social">
        <a href={discordUrl} aria-label="Metrale on Discord" target="_blank" rel="noopener"><DiscordIcon size={20} /></a>
        <a href={xUrl} aria-label="Metrale on X" target="_blank" rel="noopener"><XIcon size={17} /></a>
      </div>
    </div>
  ...
  <div class="footer-legal">
    <div>© Metrale · Pure Rust + CUDA · Community Edition AGPLv3.</div>
  </div>
</footer>
