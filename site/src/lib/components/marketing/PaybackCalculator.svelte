  ...
  <div class="av-tabs" role="tablist" aria-label="Payback scenarios">
    {#each TABS as t, i}
      ...
      <button
        ...
        class="av-tab"
        id={`payback-tab-${t}`}
        aria-controls={tab === t ? `payback-panel-${t}` : undefined}
        aria-selected={tab === t}
        tabindex={tab === t ? 0 : -1}
        onclick={() => (tab = t)}
        onkeydown={(ev) => onKey(ev, i)}>{paybackCopy[t].title}</button
      >
    {/each}
  </div>
    ...
    <div class="av-calc-grid" role="tabpanel" id="payback-panel-fleet" aria-labelledby="payback-tab-fleet">
      <div class="av-calc-inputs">
        ...
        <label class="av-field"
          ><span>GPUs in the fleet <span class="av-evidence">USER</span></span><input
            type="number"
            min="1"
            step="1"
            bind:value={f.gpus}
        ...
        >
        <label class="av-field"
          ><span>Cost per GPU per year, amortized purchase or rental <span class="av-evidence">USER</span></span><input
            type="number"
            min="0"
            step="500"
            bind:value={f.gpuCostPerYear}
          /></label
        >
        <label class="av-field">
          <span
            >Throughput uplift on your workload <span class="av-evidence is-measured">MEASURED {live.ratio} at C={live.c} on GB10</span
            ></span
          >
          <span class="av-range"
            ><input type="range" min="1" max={Math.max(1.5, measuredRatio).toFixed(2)} step="0.01" bind:value={f.uplift} /><output
              >{Number(f.uplift).toFixed(2)}×</output
            ></span
          >
        </label>
        <label class="av-field"
        ...
        <div class="av-field-row">
          <label class="av-field"
            ><span>Watts per GPU <span class="av-evidence">USER</span></span><input
              type="number"
              min="0"
              step="10"
              bind:value={f.wattsPerGpu}
            /></label
          >
          <label class="av-field"
            ><span>PUE <span class="av-evidence">USER</span></span><input type="number" min="1" step="0.05" bind:value={f.pue} /></label
          >
        </div>
        <div class="av-field-row">
          <label class="av-field"
            ><span>Dollars per kWh <span class="av-evidence">USER</span></span><input
              type="number"
              min="0"
              step="0.01"
              bind:value={f.usdPerKwh}
            /></label
          >
          <label class="av-field"
            ><span>Per GPU software replaced, per year <span class="av-evidence">USER</span></span><input
              type="number"
              min="0"
              step="100"
              bind:value={f.replacedSoftwarePerGpuYear}
            /></label
          >
        </div>
      ...
      <div class="av-calc-out">
        ...
        <dl class="av-calc-rows">
          <div>
            <dt>GPUs freed by the uplift</dt>
            <dd>{num(fr.freedGpus, 1)}</dd>
          </div>
          <div>
            <dt>Deferred purchase or rental</dt>
            <dd>{usd(fr.capacityValue)}</dd>
          </div>
          <div>
            <dt>Power no longer burned</dt>
            <dd>{usd(fr.powerValue)}</dd>
          </div>
          {#if fr.replaced}<div>
              <dt>Software replaced</dt>
              <dd>{usd(fr.replaced)}</dd>
            </div>{/if}
          <div>
            <dt>Gross savings per year</dt>
            <dd>{usd(fr.grossSavings)}</dd>
          </div>
          <div>
  ...
  {:else if tab === 'api'}
    <div class="av-calc-grid" role="tabpanel" id="payback-panel-api" aria-labelledby="payback-tab-api">
      <div class="av-calc-inputs">
        ...
        <label class="av-field"
          ><span>Current monthly API bill <span class="av-evidence">USER</span></span><input
            type="number"
            min="0"
            step="500"
            bind:value={a.monthlySpend}
          /></label
        >
        <label class="av-field"
          ><span>Blended price per million tokens, in plus out <span class="av-evidence">USER</span></span><input
            type="number"
            min="0.01"
            step="0.05"
            bind:value={a.usdPerMillionTokens}
          /></label
        >
        <label class="av-field"
          ><span
            >Tokens per second per box <span class="av-evidence is-measured"
              >MEASURED at C={live.c}, {ladderData.box.gpu.split(',')[0]}</span
            ></span
          ><input type="number" min="1" step="1" bind:value={a.boxTokensPerSecond} /></label
        >
        <div class="av-field-row">
          <label class="av-field"
            ><span>Box cost <span class="av-evidence">USER</span></span><input
              type="number"
              min="0"
              step="100"
              bind:value={a.boxCapex}
            /></label
          >
          <label class="av-field"
            ><span>Amortize over, months <span class="av-evidence">USER</span></span><input
              type="number"
              min="1"
              step="1"
              bind:value={a.amortMonths}
            /></label
          >
        </div>
        <div class="av-field-row">
          <label class="av-field"
            ><span>Utilization <span class="av-evidence">USER</span></span><span class="av-range"
              ><input type="range" min="0.1" max="1" step="0.05" bind:value={a.utilization} /><output
                >{Math.round(a.utilization * 100)}%</output
              ></span
            ></label
          >
          <label class="av-field"
        ...
        <div class="av-field-row">
          <label class="av-field"
            ><span>Watts per box <span class="av-evidence">USER</span></span><input
              type="number"
              min="0"
              step="10"
              bind:value={a.wattsPerBox}
            /></label
          >
          <label class="av-field"
            ><span>Dollars per kWh <span class="av-evidence">USER</span></span><input
      ...
      <div class="av-calc-out">
        ...
        <dl class="av-calc-rows">
          <div>
            <dt>Tokens per month on the bill</dt>
            <dd>{num(ar.tokensPerMonth / 1e6)} M</dd>
          </div>
          <div>
            <dt>Boxes needed at measured throughput</dt>
            <dd>{ar.boxes}</dd>
          </div>
          <div>
            <dt>Amortized hardware per month</dt>
            <dd>{usd(ar.capexPerMonth)}</dd>
          </div>
          <div>
            <dt>Power per month</dt>
          ...
          <div class="is-total">
            ...
            <dd>{usd(ar.newMonthly)}</dd>
          </div>
          <div>
            <dt>Cost per million tokens</dt>
            <dd>{usd(ar.costPerMillion, 3)}</dd>
          </div>
          <div>
            <dt>Monthly savings</dt>
            <dd>{usd(ar.monthlySavings)}</dd>
          </div>
        </dl>
      </div>
    </div>
  {:else}
    <div class="av-calc-grid" role="tabpanel" id="payback-panel-energy" aria-labelledby="payback-tab-energy">
      <div class="av-calc-inputs">
        ...
        <div class="av-field-row">
          <label class="av-field"
            ><span>Metrale, tokens per second <span class="av-evidence is-measured">MEASURED C={maxC}</span></span><input
              type="number"
              min="1"
              step="1"
              bind:value={e.tokensPerSecond}
            /></label
          >
          <label class="av-field"
            ><span>{live.baselineLabel}, tokens per second <span class="av-evidence is-measured">MEASURED C={maxC}</span></span><input
              type="number"
              min="1"
              step="1"
              bind:value={e.baselineTokensPerSecond}
            /></label
          >
        </div>
        <div class="av-field-row">
          <label class="av-field"
            ><span
              >Metrale, watts under load <span class="av-evidence" class:is-measured={curve.measured}
                >{curve.measured ? 'MEASURED' : 'USER'}</span
              ></span
            ><input type="number" min="1" step="5" bind:value={e.watts} /></label
          >
          <label class="av-field"
            ><span
              >{live.baselineLabel}, watts under load
              <span class="av-evidence" class:is-measured={curve.measured}>{curve.measured ? 'MEASURED' : 'USER'}</span></span
            ><input type="number" min="1" step="5" bind:value={e.baselineWatts} /></label
          >
        </div>
        <label class="av-field"
          ><span>Tokens served per month, millions <span class="av-evidence">USER</span></span><input
            type="number"
            min="0"
            step="100"
            bind:value={e.millionTokensPerMonth}
          /></label
        >
        <div class="av-field-row">
          <label class="av-field"
            ><span>PUE <span class="av-evidence">USER</span></span><input type="number" min="1" step="0.05" bind:value={e.pue} /></label
          >
          <label class="av-field"
            ><span>Dollars per kWh <span class="av-evidence">USER</span></span><input
              type="number"
      ...
      <div class="av-calc-out">
        <div class="av-calc-hero">
          ...
          <span class="av-small"
            >{#if er.ratio === null}no baseline to compare{:else if er.energyCutPct > 0}against {num(er.baselineTokensPerJoule, 2)}, so {num(
                er.energyCutPct,
                1
              )}% less energy for the same token{:else if er.energyCutPct < 0}against {num(er.baselineTokensPerJoule, 2)}, so the baseline
              is the more efficient at these inputs{:else}against {num(er.baselineTokensPerJoule, 2)}, no difference at these inputs{/if}</span
          >
        </div>
        ...
        <table class="av-calc-table">
          ...
          <tbody>
            ...
            <tr
              ><th scope="row">kWh per million tokens</th><td>{num(er.kwhPerMillion, 3)}</td><td>{num(er.baselineKwhPerMillion, 3)}</td></tr
            >
            <tr
              ><th scope="row">Energy cost per million tokens</th><td>{usd(er.usdPerMillion, 4)}</td><td
                >{usd(er.baselineUsdPerMillion, 4)}</td
              ></tr
            >
        ...
        <dl class="av-calc-rows">
          <div class="is-total">
            <dt>kWh not burned per month</dt>
            <dd>{num(er.kwhSavedPerMonth)}</dd>
          </div>
          <div>
            <dt>Energy bill saved per year</dt>
            <dd>{usd(er.usdSavedPerYear)}</dd>
          </div>
        </dl>
      </div>
      <figure class="av-calc-chart">
        ...
        <figcaption>
          ...
          <span class="av-evidence" class:is-measured={curve.measured}
            >{curve.measured ? `MEASURED throughput and draw, ${box}` : `MEASURED throughput, ${box}. USER draw`}</span
          >
...
<style>
  ...
  .av-calc-inputs .av-field > span:first-child {
    font-size: 0.84rem;
    font-weight: 600;
    color: var(--t1);
  }
  ...
  .av-calc-table th[scope='row'] {
    text-align: left;
    font-weight: 400;
    color: var(--t2);
    padding-right: 0.75rem;
  }
  .av-calc-table th[scope='col'] {
    font-family: var(--font-mono);
    font-size: 0.7rem;
    font-weight: 600;
