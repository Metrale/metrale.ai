...
// a record on the retired 512/320 instrument refuses to pair — and rendering
// the tab on such a record keeps the fallback's wording under test.
//
// Every dense record this site publishes is on the published instrument, so
// the retired one is rebuilt from the newest real record with only its
// instrument put back (ISL 512 / OSL 320, the natural fixture, ctx 4096) and
// its throughput at the quarter the old instrument measured.
const retiredLatest = {
  ...denseLatest,
  git_sha: 'retired000',
  recorded_at: denseLatest.recorded_at - 86_400,
  params: { ...denseLatest.params, isls: '512', osl: '320', prompt_mode: 'natural' },
  serve_overrides: { kv_cache_dtype: 'fp8', max_batch_size: '128', max_model_len: '4096' },
  metrics: Object.fromEntries(
    Object.entries(denseLatest.metrics)
      .filter(([k]) => /aggregate_tok_s$/.test(k))
      .map(([k, v]) => [k, v / 4])
  ),
};
const rfRetired = (bench) => (bench === 'concurrency-sweep' ? [retiredLatest] : recordsFor(bench));
// The MoE subject before its first gate record: a vLLM one-shot and nothing
// else. The published records carry MoE runs, so that state is rendered on
// the records with those runs taken out.
const rfNoMoe = (bench) => (bench === 'concurrency-sweep-moe' ? [] : recordsFor(bench));
...
describe('the subject strip', () => {
  ...
  test('"not yet measured" sits INSIDE the tab button of every subject with no records', () => {
    const page = renderTab('qwen38-27b', rfNoMoe);
  ...
  test('records no subject claims are listed, never dropped', () => {
    ...
    expect(renderTab('qwen38-27b', withExtra([stray]))).toContain('unassigned: concurrency-sweep · nvidia/Qwen3.6-35B-A3B-NVFP4 (1)');
  });
});
...
describe('the MoE tab with no gate record: a vLLM one-shot on the published instrument, no Metrale Engine leg', () => {
  ...
  test('the vLLM series is drawn as squares at its measured rungs only, every value from the ladder', () => {
    ...
    for (const r of vllm.rungs)
      expect(svg).toContain(
        `<title>vLLM + MTP · C=${r.c} · ${r.tok_s.toFixed(2)} tok/s · mean of ${r.reps} reps · spread ${r.spread_pct}% · ${r.source}</title>`
      );
  ...
  test('C=32/64/128 are absent with the recorded reason where the point would be — no zero, no interpolation', () => {
    ...
    const svg = comparisonSvg(page);
    for (const c of vllm.unmeasured.rungs) expect(svg).toContain(`<title>C=${c} · not measured — ${vllm.unmeasured.reason}</title>`);
...
describe('the DFlash tab today: Metrale Engine only, absent rungs answered in place', () => {
  ...
  test('the legend and caption say vLLM was not run on THIS instrument, and what would fill it', () => {
    ...
    expect(page).toContain(
      'class="gate-panel-title">Metrale Engine vs vLLM · gate instrument · ISL 512 / OSL 200 · natural fixture · batch cap 16 · fp8 KV<'
    );
    inOrder(
      page,
...
describe('the dense tab today: the live gate record paired with the published vLLM bar', () => {
  // ★ WHAT SHIPS. The newest passing concurrency-sweep record on
  ...
  test("the gate record's numbers are drawn on the comparison chart; the frozen August Metrale Engine series is not", () => {
    ...
    expect(svg).toContain(
      `<title>Metrale Engine · C=128 · ${gateC128} tok/s · gate record ${fmtDate(live.recorded_at)} · ${live.git_sha} · click for record</title>`
    );
...
describe('the dense tab FALLBACK: the published pair when no live record pairs', () => {
  ...
  // tab shipped in before the gate was re-pointed, and the state it returns to if the live series
  ...
  test('the dated stamp is INSIDE the ladder svg, at the last rung of each vLLM line, and never overprints', () => {
    ...
    const stamp = (date) =>
      +new RegExp(`<text class="gc-ref-label cl-stamp" x="740" y="([\\d.]+)" text-anchor="end">vLLM 0.27.1 · ${date}</text>`).exec(
        svg
      )?.[1];
...
describe('the comparison state, decided once', () => {
  ...
  test('a declared manifest without a generated ladder is said out loud', () => {
    const page = html(Comparison.default, {
      subject: { ...MOE, id: 'moe-x', published_manifest: 'bench/moe/published.json' },
      records: [],
      rungs,
      onselect: () => {},
    });
  ...
  test('instrument, range and batch-cap readers print only what is recorded', () => {
    ...
    expect(measuredRange([{ measured_utc: '2026-08-18T15:20:55Z' }, { measured_utc: '2026-08-17T01:36:35Z' }])).toBe(
...
describe('the published pair: series pills and the ladder they drive', () => {
  ...
  test('hiding the tall series rescales the y axis to what is still drawn', () => {
    ...
    const max = (ids) => Math.max(...series.filter((s) => ids.includes(s.id)).flatMap((s) => s.rungs.map((r) => r.tok_s)));
...
// ---------------------------------------------------------------------------
// The metric is called ITL on the page
//
...
describe('ITL naming', () => {
  ...
  test('the record key is still tpot_p50_ms — the rename is display-only', () => {
    const rung = publishedLadder?.subjects
      ? Object.values(publishedLadder.subjects).flatMap((s) => Object.values(s.series ?? {}).flatMap((x) => x.rungs ?? []))[0]
      : null;
