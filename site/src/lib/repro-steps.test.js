...
const CTX = {
  generated: { sha: '4b18f7cec', date: '2026-09-16' },
  meta: { expected_secs: 1560, sensitivity: 'Speed' },
...
const sweep = () => ({
  ...
  engine_version: '1.0.0-beta-preview',
...
const shardRec = (index, count, extra = {}) => ({
  ...
  metrics: {
    ...
    samples: 10,
    overall_accuracy: 90,
    'subset.simple_python.hits': 9,
    'subset.simple_python.n': 10,
  },
  verdict: 'info',
...
describe('shell safety', () => {
  ...
  it('renders a bare boolean flag on its own line', () => {
    ...
    expect(splitFlags(argv).pairs).toEqual([
      ['--param', 'iterations=10'],
      ['--yes', null],
      ['--pull-request-gate', null],
    ]);
...
describe('a partition aggregate reproduces as N shard commands', () => {
  ...
  it('never presents the inherited single command as the run', () => {
    ...
    expect(
      runs.filter((c) => splitShell(c.lines.join('\n')).slice(0, inheritedArgv.length).join('\0') === inheritedArgv.join('\0'))
    ).toHaveLength(1);
...
describe('a lone shard record', () => {
  it('derives the sibling commands, marks them derived, and adds the aggregate', () => {
    ...
    expect(siblings.map((c) => c.lines.find((l) => l.includes('shard=')))).toEqual([0, 1, 2, 3, 4].map((j) => `  --param shard=${j}/6 \\`));
...
describe('derived flags are appended and reported', () => {
  ...
  it('adds --checkpoint and --hardware as the last, marked lines', () => {
    ...
    expect(plan.derived).toEqual([
      { flag: '--checkpoint', from: 'target_model' },
      { flag: '--hardware', from: 'perf_class' },
    ]);
    ...
    expect(plan.script).toContain(
      '# lines added by this page, not in the record: --checkpoint (from target_model), --hardware (from perf_class)'
    );
  });
...
describe('caveats', () => {
  ...
  it('an unsigned record and an invalidated box state are said, a valid one is not', () => {
    const plan = reproSteps(
      { ...sweep(), signer: null, box_state: { validity: 'invalidated', concerns: ['throttled'], elapsed_s: 5 } },
      CTX
    );
