...
/**
 * The chips (.av-chip-*) tint their ground with 12% of the fill hue, and the
 * twin sits on that tint. Over --card-2 it is the tightest pair on the site:
 * on 2026-09-21 Lighthouse read the violet chip at 4.44:1 with twins that
 * cleared every plain ground, so the tinted grounds are checked as well.
 * color-mix(in srgb, hue 12%, transparent) over a ground composites to
 * 0.12 * hue + 0.88 * ground, channel by channel.
 */
...
for (const [textName, fillName] of TWINS) {
  ...
  test(`--${fillName} is a hex literal, so the chip tint can be computed`, () => {
    expect(fill).toMatch(/^#[0-9a-fA-F]{6}$/);
  });
  for (const [groundName, ground] of GROUNDS) {
    const tinted = tint(fill, ground, CHIP_TINT);
    test(`light --${textName} (${lightHex(textName)}) clears ${FLOOR}:1 on the chip tint over ${groundName} (${tinted})`, () => {
      expect(contrast(lightHex(textName), tinted)).toBeGreaterThanOrEqual(FLOOR);
    });
  }
}
