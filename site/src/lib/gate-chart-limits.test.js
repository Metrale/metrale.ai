...
const violations = (page) => [...plot(page).matchAll(/<circle class="gc-viol" cx="([\d.]+)" cy="([\d.]+)" r="7\.5" data-limit="(\w+)"/g)];
...
describe('GateChart: a ceiling', () => {
  const wall = (v, budget) => rec({ benchmark_id: 'agentic-webserver', metrics: { sum_wall_s: v }, params: { wall_budget_s: budget } });
  const page = html(GateChart, {
...
describe('GateChart: a declared ceiling applies only from the date it took effect', () => {
  ...
  test('NEGATIVE CONTROL: every point dated after the ceiling took effect is judged by it', () => {
    const all = html(GateChart, {
      panel,
      records: [ttft(since + DAY, ceiling * 2), ttft(since + 2 * DAY, ceiling * 2)],
      onselect: () => {},
    });
...
describe('GateChart: a metric with no limit anywhere', () => {
  test('draws no rule, no label and no legend key', () => {
    const page = html(GateChart, {
      ...
      records: [
        rec({ benchmark_id: 'ttft-warm-gate', metrics: { median_ms: 400 } }),
        rec({ benchmark_id: 'ttft-warm-gate', metrics: { median_ms: 410 } }),
      ],
      onselect: () => {},
    });
