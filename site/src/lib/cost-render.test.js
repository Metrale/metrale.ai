...
const rung = (c, o = {}) => {
  ...
  // verified, and it is drawn hollow, correctly. Tests that mean
...
describe('the populated state, which is what ships', () => {
  // The REAL committed data and the REAL generated ladders. Now the
...
describe('a losing rung is drawn, labelled and counted against us', () => {
  ...
  test('a mixed ladder shows both rungs and counts only the won one', () => {
    const mixed = panel([rec({ ...cell(1), ...cell(8, { watts: 20 }) })], ladders([rung(1), rung(8)]));
  ...
  test('every lose label stays INSIDE the plot — a clipped factor is a loss not shown', () => {
    ...
    const wide = panel([rec(Object.assign({}, ...cs.map((c) => cell(c))))], ladders(cs.map((c) => rung(c))), 128);
    const svg = plot(wide);
...
describe('a window whose sampler cadence was never recorded', () => {
...
describe('facility overhead', () => {
  ...
  test('the MARK SITS ON THE LOW EDGE — never floating inside an assumption', () => {
    ...
    const onLow = [...back.matchAll(/L([\d.]+) ([\d.]+)/g)].map((m) => `${Number(m[1]).toFixed(1)},${Number(m[2]).toFixed(1)}`);
