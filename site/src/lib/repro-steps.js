...
export function reproSteps(record, ctx) {
  ...
  if (sha) {
    const lines = [`git clone ${ENGINE_REPO}.git metrale-engine && cd metrale-engine`];
  ...
  if (!rec.engine_version) miss('engine version', 'the binary version string; compare `met --version` after building');
  ...
  const warmup = {
    id: 'warmup',
    title: 'Warm-up & probe',
    summary: warmFacts.length
      ? warmFacts.map(([k, v]) => `${k.split('.').pop()} ${v}`).join(' · ')
      : 'this benchmark records no warm-up parameters',
    facts: warmFacts,
    commands: [],
  ...
  if (partition) {
    ...
    if (shard) {
      for (let j = 0; j < shard.count; j += 1) {
        if (j !== shard.index)
          addRun(
            `sibling shard ${j + 1} of ${shard.count} (derived: shard=${j}/${shard.count} substituted)`,
            withShard(rec.command, j, shard.count),
            true
          );
      }
      measure.commands.push({
  ...
  if (rec.params && ['non_live_pct', 'live_pct', 'hallucination_pct'].every((k) => k in rec.params)) {
    ...
    if (!rec.dataset_fingerprint)
      miss(
        'dataset_fingerprint',
        'the content hash of the provisioned BFCL draw; check your set reports the same sample count before comparing'
      );
  }
  measure.notes.push(
  ...
  if (derived.length) {
    measure.notes.push(
      `Marked lines were added by this page and are not in the record: ${derived.map((d) => `${d.flag} from ${d.from}`).join(', ')}.`
    );
  }
  ...
  if (partition)
    judge.notes.push(
      'Each shard record reads `info`; this verdict was re-derived by the dashboard from the aggregate against params.min_* exactly as the gate does.'
    );
  ...
  if (rec.box_state) {
    ...
    if (b.validity && b.validity !== 'valid' && b.validity !== 'not-applicable')
      caveats.push(`Box state ${b.validity}: SPEED numbers in this record are not comparable.`);
  } else {
  ...
  return {
    headline,
    preface: { required, costs, expect },
    steps,
    missing,
    caveats,
    derived,
    script: toScript(steps, headline, missing, caveats, derived),
  };
}
