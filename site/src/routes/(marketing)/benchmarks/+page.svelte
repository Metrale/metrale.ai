<script>
  ...
  import { groupFor, groupRecords } from '$lib/gate-variants.js';
...
<PageShell path={routes.benchmarks}>
  ...
  >
    <div class="av-grid av-grid-4 av-bench-stats">
      <div class="av-tile">
        <span class="av-tile-label">At C={live.c}</span>
        <div class="av-num">{live.ratio}</div>
        <p>the matched vLLM configuration, same GB10</p>
      </div>
      <div class="av-tile">
      ...
      <div class="av-tile">
        <span class="av-tile-label">Rungs</span>
        <div class="av-num">{live.won}/{live.rungs}</div>
        <p>won against matched vLLM + MTP</p>
      </div>
      <div class="av-tile">
        <span class="av-tile-label">Gate records</span>
        <div class="av-num">{src.committed + src.from_branches}</div>
        <p>{src.committed} committed, {src.from_branches} from {src.branches_scanned} branches</p>
      </div>
    </div>
  </PageHero>
  ...
  <section class="av-section" id="gates">
    <div class="av-container">
      ...
      <p class="av-small av-reveal" style="margin-top:1rem">
        Repro: <code class="av-mono">{bench.repro_cmd}</code> · generated at {gateData.generated_sha}, {gateData.generated_date}
      </p>
    </div>
  </section>
  ...
  <CtaBand
    title="Run the ladder on your workload."
    body="A side by side against your current engine on your hardware inside 48 hours, with the campaign log to match."
    primary={{ text: 'Book a demo', href: routes.demoForm }}
    secondary={{ text: 'Verification walkthrough', href: routes.diligence }}
  />
</PageShell>
