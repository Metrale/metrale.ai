...
<PageShell path={routes.labs}>
  ...
  <section class="av-section av-section-alt">
    <div class="av-container">
      ...
      <div class="av-grid av-grid-4 av-reveal">
        {#each l.tracks as t}
          <div class="av-card av-card-accent av-sx-{t.color}">
            <h3 style="font-size:1rem">{t.title}</h3>
            <p style="font-size:0.9rem">{t.body}</p>
          </div>
        {/each}
      </div>
    </div>
  ...
  <section class="av-section">
    <div class="av-container av-narrow">
      <div class="av-status-note av-reveal" style="margin:0 0 1.5rem">
  ...
  <CtaBand
    ...
    body="Kernels, compilers, protocols, benchmarks, safety evaluation. If it advances inference on hardware people own, we want the conversation."
    primary={l.cta}
